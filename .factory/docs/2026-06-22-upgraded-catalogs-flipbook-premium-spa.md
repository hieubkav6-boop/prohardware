...
- **Vấn đề:** Trang Catalogs cũ tải rất chậm, ảnh JPEG trích xuất từ PDF quá nặng, giao diện CRUD ở trang quản trị rườm rà và trang khách không hỗ trợ phân loại theo danh mục hay trải nghiệm lật sách chuẩn 3D.
- **Giải pháp:**
  ...
  3. Xây dựng sidebar dạng Accordion nhóm theo danh mục, tự động mở danh mục đầu tiên và tự nạp trước (pre-load) trang bìa của tất cả tài liệu.
  4. Nâng cấp bộ đọc sách lật (`CatalogFlipbook`) hỗ trợ Toolbar phóng to/thu nhỏ, xem 1 hoặc 2 trang, hiển thị gáy sách đổ bóng 3D chân thực, và tích hợp cơ chế nạp trước ảnh 4 cấp độ (Tier 1-4) theo hành vi người dùng (cuộn trang, di chuột hover).
...
- **Hình ảnh so sánh:** Giống như một người phục vụ nhà hàng chuyên nghiệp. Thay vì đợi khách gọi món rồi mới chạy vào bếp làm (khiến khách phải chờ lâu), người phục vụ sẽ chuẩn bị sẵn các đĩa khai vị lên bàn (Tier 1), khi thấy khách nhìn vào món chính nào thì chuẩn bị sẵn nguyên liệu món đó (Tier 2), và khi khách đang ăn món chính thì chuẩn bị sẵn món tráng miệng (Tier 3-4).
...
- `Sửa:` [app/admin/catalogs/page.tsx](file:///e:/NextJS/job/job_from_system_vietadmin/prohardware/app/admin/catalogs/page.tsx): Tích hợp ô nhập danh mục và nâng cấp thuật toán trích xuất PDF từ `image/jpeg` sang `image/webp`.
...
2. Cấu hình ô input Danh mục trong admin và thay đổi kiểu nén ảnh canvas sang WebP.
3. Tạo hook quản lý tải trước tài nguyên ảnh.
4. Tái cấu trúc sidebar client hiển thị theo nhóm danh mục dạng accordion.
5. Cập nhật trình sách lật Flipbook hỗ trợ toolbar zoom và skeleton mượt mà.
6. Chạy typecheck và kiểm tra tổng thể.
...
- **Kiểm chứng thủ công:**
  1. Vào `/admin/catalogs`, tải lên file PDF và kiểm tra log trích xuất thành WebP trong Network Tab của DevTools.
  2. Vào `/catalogs`, kiểm tra sidebar hiển thị accordion đóng mở mượt mà và việc prefetch ảnh bìa cũng như ảnh các trang sau khi hover chuột.
...
- [x] Cập nhật trang admin `app/admin/catalogs/page.tsx`
...
- Trải nghiệm chuyển đổi chế độ xem Single/Double linh hoạt theo kích thước màn hình.
