...
- **Vấn đề:** Trang Catalogs cũ tải rất chậm, ảnh JPEG trích xuất từ PDF quá nặng, giao diện CRUD ở trang quản trị rườm rà (yêu cầu điền slug, SEO, ảnh bìa thủ công...) và trang khách không hỗ trợ phân loại theo danh mục hay trải nghiệm lật sách chuẩn 3D.
- **Giải pháp:**
  ...
  3. **Tối giản hóa biểu mẫu (KISS):** Ẩn hoàn toàn trường nhập "Đường dẫn tĩnh (Slug)" khỏi giao diện admin. Slug được tự động sinh ngầm từ tiêu đề ở phía sau hậu trường. Thay thế hiển thị slug trên danh sách bằng tên Danh mục thực tế.
...
- **Lược bỏ gánh nặng nhập liệu:** Bản chất hệ thống chỉ chạy trên duy nhất 1 trang `/catalogs` cho toàn bộ tài liệu (Single-Page App). Do đó, quản lý slug chi tiết là hoàn toàn dư thừa với người quản trị. Chúng tôi ẩn trường này đi, tự sinh tự động để giữ cho form cực kỳ gọn và tập trung vào các trường quan trọng (Tiêu đề, Danh mục, File PDF).
...
- Ẩn trường Slug khỏi giao diện người dùng và thay thế bằng hiển thị tên Danh mục trên danh sách quản trị để tăng tính trực quan.
...
- `Sửa:` [app/admin/catalogs/page.tsx](file:///e:/NextJS/job/job_from_system_vietadmin/prohardware/app/admin/catalogs/page.tsx): Tích hợp ô nhập danh mục, loại bỏ input Slug khỏi UI, tự động sinh slug từ tiêu đề, nâng cấp thuật toán trích xuất PDF từ `image/jpeg` sang `image/webp`.
...
2. Cấu hình ô input Danh mục trong admin, loại bỏ trường nhập Slug.
...
- **Kiểm chứng thủ công:**
  1. Vào `/admin/catalogs`, tải lên file PDF và kiểm tra xem có tạo/sửa catalog thành công mà không cần nhập slug hay không.
...
- [x] Cập nhật trang admin `app/admin/catalogs/page.tsx` (Ẩn nhập Slug, tự động xử lý ngầm)
...
- Không hiển thị ô nhập liệu Slug cho người dùng.
